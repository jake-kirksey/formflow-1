...
export function WorkflowView(props) {
  ...
  // console.log("CSV Data in WorkflowView:", csvData);
  ...
  return (
    ...
    >
      <div
        className={"md:flex gap-12 flex-1 md:h-full space-y-10 md:space-y-0"}
      >
        <div
          className={
            "flex gap-6 md:gap-0 flex-col-reverse md:flex-col justify-between overflow-y-auto p-2"
          }
        >
          <SchemaBasedForm
          ...
          <div className={"rounded-lg shadow-md p-3 md:mt-3"}>
            <h1 className={"font-bold text-lg"}>Job info</h1>
        ...
        <div className={"h-96 md:h-full md:flex-1 overflow-y-auto"}>
          {props.submissions && (
            <SubmissionHistoryView
              ...
              keysToLabelsMap={Object.assign(
                {},
                ...props.workflow.submissionFields.map((field) => ({
